...
當你發現氣味定義中出現「以不同方式解決相同問題」的字眼，可能會開始懷疑它是否與「重複程式碼」程式碼異味相似。如果我們比較「歧異解方」和「重複程式碼」兩種氣味，我們可以發現「歧異解方」指的是**不同的**程式碼片段卻解決了**相同的**問題；另一方面，重複程式碼則是由**相似的**程式碼（或完全相同的程式碼）用於解決某些問題。請留意這裡「重複程式碼」不一定需要用於解決**相同**的問題；有時它只是意外地重複了。這種重複通常是由複製貼上所引起的。
